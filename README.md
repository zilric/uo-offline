...
- **Play with friends.** Your PC can host and friends can join your world. Pick "Host for friends" or "Join a friend's game" in the installer, or switch an install later with `friends.bat` (`friends.sh` on Linux). A join install is client only, so it is quick. Same house works over the LAN; anywhere else works over Tailscale with no router changes. See [docs/FRIENDS.md](docs/FRIENDS.md). Fixed on the way: the server had been looking up a public address at every boot because of a misspelled setting; it no longer touches the internet.
- **Recruit bots into your guild.** Recruit a bot at your guildstone and it joins a few seconds later, the way the recruit would have walked to the stone. A bot in your guild wears your tag and never leaves the shard: it is not logged out, not purged at boot, and not swept by a regeneration, and it survives a save and restart. Guild chat works now too, and typing "anyone wanna group up" in it gets a few guildmates saying "yeah", recalling to you with a real Recall, and joining your party. Your guild bots talk in guild chat on their own while you are online: random chatter, "what is everyone doing" answered with what each one is really doing and where, and a bot asking for a group of its own, which its guildmates recall to and set out with. Say "yeah" to a bot's call and they come to you instead.
- **Thieves pick pockets.** The Thief class had the skills and the dark leathers and never used them. A thief now works the crowd at banks, taverns and shop doors, walks up to a mark, sometimes hidden and stealthing, peeks in the pack and lifts the best light thing in it, from bots and from you. It is the real Stealing skill, so the weight cap, the skill roll, the caught roll and the criminal flag are all the engine's. A caught thief says nothing, runs and hides; caught next to a banker it dies to the guards in about a second, the same as 1999. Most thieves belong to The Thieves Guild [TG], a bot guild nobody else can roll into. Thieves work dungeons too: one that walks through a dungeon door sneaks between the rooms on that floor robbing adventurers between their fights, hides from monsters instead of fighting them, and after a lift walks hidden to the stairs and out to a bank. Every lift goes straight to the bank box, victims gossip about being robbed, and a bot that raises a flagged thief in town no longer gets whacked for it.
...
Your PC runs the world; friends run the game client and connect to it. In the installer, pick **Host for friends** on your PC and **Join a friend's game** on theirs, typing the address your `friends.bat` shows. An install you already have switches with `friends.bat host`, `friends.bat solo`, or `friends.bat join ADDRESS NAME PASSWORD` (`./friends.sh` on Linux and the Steam Deck).

On the same LAN the address is your local IP. From anywhere else, put [Tailscale](https://tailscale.com) on both PCs and use the `100.x.y.z` address it gives you; nothing is opened to the internet and no router changes are needed. Windows asks once to open port 2593 in its firewall when you host. Friends can only play while your game is running, and their characters live in your save. Full steps and fixes for "cannot connect" are in [docs/FRIENDS.md](docs/FRIENDS.md).
