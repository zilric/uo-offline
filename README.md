...
- **Play with friends.** Clicking UO Offline now asks how you want to play: by yourself, host for friends, or join a friend. Hosting lets friends into your world while you play, same house over the LAN or anywhere over Tailscale with no router changes; joining connects the game to a friend's PC with a name and password you pick. Nothing about the install changed. See [docs/FRIENDS.md](docs/FRIENDS.md). Fixed on the way: the server had been looking up a public address at every boot because of a misspelled setting; it no longer touches the internet.
