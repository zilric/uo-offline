...
- **Unified High-Density Frontier Seeder (`WorldFrontierSeeder.cs`):**
  - Scaled candidate evaluation budget to 250,000 attempts at 50 checks per 35ms tick, achieving clean, high-throughput placements across the entire world (including Dagger Isle's own snow plains).
  - Calibrated commercial saturation down to a dynamic 5%-8% vendor ratio to prioritize authentic residential settlements.
  - Implemented real-time GM broadcasts and console reporting at every 10% milestone interval.
  - Deprecated and purged the legacy anchor-relative seeders (`WorldHouseSeeder.cs`, `DaggerIsleSeeder.cs`) in favor of this single unified pipeline.
- **Market House Directory (`OrganicMarketDirectoryGump.cs`):**
  - Fixed a GUMP button ID collision for catalogs exceeding 1,000 properties by widening action stride offsets to 1,000,000.
  - Added reverse-mapped, human-readable house type labels beside each entry's world coordinates.
  - Added `<< Prev 10` and `Next 10 >>` fast-skip pagination controls for large directories.
...
- **Unified Admin Interface (`[vh` / `OrganicMarketAdminGump.cs`):**
  - Streamlined the action palette down to a single unified `Seed World (Frontier Density)` pipeline, and cleaned the now-obsolete legacy seeder command registrations out of `CommandPanelRegistry.cs`.
...
- **Placement Spacing Configuration:**
  - Decoupled placement conflict margins into dedicated directional buffers (`SideToSideBuffer = 3`, `FrontToBackBuffer = 5` tiles), the default used by manual/interactive house placement - the automated `WorldFrontierSeeder` pipeline above overrides these to a tighter `2`/`4` for its own high-density passes.
